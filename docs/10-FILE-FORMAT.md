...
- `manifest.json` MUST be the first entry in the archive and MUST parse standalone:
  `{ "format": "kiriko-project", "schema_version": "…", "written_by": "kiriko x.y.z",
...
- **Unknown-field preservation is mandatory**: a reader keeps any keys it does not
  ...
  Kiriko versions coexist (K-065) and lets Placeholder effects round-trip
...
- Deleting any or all of the sidecar at any time MUST be safe: Kiriko rebuilds on demand.
