...
**Status: canonical.** This document specifies how After Effects projects come into Kiriko.
...
|---|---|---|---|
| **Kiriko Bridge** (primary) | After Effects installed, any recent version | High — the scripting DOM is documented public API | "Imported from After Effects" |
...
All three routes converge on the same import pipeline: produce a Kiriko project fragment,
...
as a `.zxp` from the Kiriko site and repository, licensed GPLv3 with the rest of the
project. The panel walks `app.project` via the scripting DOM and writes a **Kiriko bundle**:
a folder (or zip) containing a versioned JSON document plus an optional footage collection.

The Bridge MUST NOT require Kiriko to be installed on the same machine, and MUST NOT
require network access. It writes a bundle; Kiriko opens bundles. A studio can export on
...
3. **Layers**, per comp, in stacking order — type (footage, solid, precomp, text, shape,
   ...
   reference) and the legacy layer-above form, normalised to Kiriko's matte model
...
5. **Keyframes, exactly** — per key: time, value, per-side interpolation type (linear /
   ...
   Because Kiriko keyframe maths is AE-compatible (K-025), this is a value copy, not a
...
6. **"Time Remap" keyframes** — exported as an ordinary keyframed property; the importer
   ...
   This mapping is exact: AE's "time remap" value graph and Kiriko's Retime value graph are
...
8. **Expressions** — the source text of every enabled expression, verbatim, plus the
   enabled/disabled state. Never evaluated, never rewritten by the Bridge. Kiriko decides
...
9. **Effects** — per instance: match name, display name, enabled state, and a full
   ...
   §5; parameter match names included). The Bridge does not know which effects Kiriko can
...
10. **Text layers** — source text (including keyframed source text), character/paragraph
    ...
    their selectors and animated properties. Animators import to the extent Kiriko's text
    engine supports them (see matrix); the full structure is preserved in the bundle either
    way so later Kiriko versions can re-import without re-exporting.
...
`project.json` is expressed **in the Kiriko project schema** defined in
...
On opening a bundle (or a parsed `.aep`), Kiriko resolves footage in this order: collected
...
- **Keyframes** are copied value-for-value (K-025): interpolation types, per-side speed and
  influence, spatial tangents, roving flags. Kiriko's evaluator reproduces AE's cubic
...
- **"Time remapping" → Retime**, losslessly, per §2.2 item 6. AE's "time stretch" maps to
  Kiriko's Stretch, including its keyframe-rescaling behaviour and negative (reversed)
  values.
- **"Track mattes" → mattes.** Both AE generations normalise to Kiriko's model (chosen
...
- **Collapse**: AE's "collapse transformations / continuously rasterize" switch maps to
  Kiriko's collapse switch on Precomp layers; the render-order consequences match because
  Kiriko's compositor implements the same semantics
  ([06-RENDER-PIPELINE.md](06-RENDER-PIPELINE.md)).
- **3D** maps onto Kiriko's 2.5D (K-023): 3D layer flags, cameras (both node types),
...
- **Colour**: AE projects are 8/16/32-bpc project-wide with an optional linearised working
  space; Kiriko is scene-linear fp16/fp32 per comp (K-026). Values convert on import;
...
- **lossless** — evaluates identically in Kiriko; no visual difference by construction.
...
| Masks (path, feather, opacity, expansion, modes) | lossless | Variable-width feather: mapped — approximated until Kiriko ships it |
...
| Blend modes — Dissolve, Dancing Dissolve | mapped | Import as Normal, flagged; no Kiriko equivalent yet |
...
| Precomps + collapse | lossless | Collapse edge cases (effect-on-collapsed-layer) follow Kiriko's compositor; flagged when AE would have broken collapse |
| Solids, nulls, guide layers | lossless | |
| Markers (comp + layer) | lossless | |
| AE built-in effects with a Kiriko equivalent | mapped | Via the match-name table (§5); parameters and keyframes carried; pixel output near-identical, not bit-identical |
...
| Cameras, lights, 3D layer flags | mapped | → Kiriko 2.5D (K-023); depth of field mapped; renderer-specific shading differs |
...
| Layer styles | placeholder | Until Kiriko ships equivalents |
...
| Render queue / output modules | unsupported | Deliberate; Kiriko's export queue is its own thing |
...
Kiriko maintains a versioned data file (`ae-effect-map.toml`, shipped with the app and
updatable independently) mapping AE effect **match names** to Kiriko built-in effects
([08-EFFECTS.md](08-EFFECTS.md)) with per-parameter correspondence and unit/range
conversion. Seeded with the montage staples:

| AE effect (match name) | Kiriko effect |
|---|---|
...
MUST have a golden-frame test: the AE-rendered frame and the Kiriko-rendered frame of a
...
- keeps the original display name, match name, enabled state, and the **complete parameter
  dump including keyframes and expressions** (parameters are real Kiriko properties: they
...
across versions without documentation. Kiriko builds on the community reverse-engineering
...
(Go, explicitly partial) — reimplemented in Rust inside `kiriko-project`, with licence
...
implementation, and a large template ecosystem. Kiriko imports Lottie shape/text/image/
...
do not arrive, and Lottie features Kiriko lacks (specific layer effects) follow the same
...
- **Loading `.aex` AE plugins — never.** Modern AE plugins depend on SmartFX, AEGP suites,
  ...
  while the same vendors already ship OFX builds. Kiriko routes all plugin demand through
...
- **Guaranteed visual parity of any comp using unmapped effects.** The matrix is the
  contract; placeholders are the mechanism; the report is the disclosure. Kiriko does not
...
- **Blend-mode gap**: do Stencil/Silhouette, Alpha Add, and Luminescent Premul earn real
  Kiriko implementations (upgrading four matrix rows to lossless), and where do they sit in
  [06-RENDER-PIPELINE.md](06-RENDER-PIPELINE.md)'s compositing order?
- **Text animator scope**: which selector modes and per-character-3D features Kiriko's text
