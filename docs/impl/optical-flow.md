...
selects), then box-downsample ×2 per level to ~16 px min dimension (≈ 6 levels at 1080p).
Also build Sobel gradient textures per level (Rg16Float: dx, dy).

**Per level, coarse → fine:**

1. **Init**: upsample flow from coarser level (bilinear, ×2 magnitude).
2. **Inverse search (the core)**: for each 8×8 patch on a stride-4 grid, refine its flow
   ...
   `residual r = Σ g·(B(x+u) − A(x))`, `Δu = H⁻¹ r`, `u += Δu`, stop when `|Δu| < 0.02 px`.
   One workgroup per patch; patch pixels in shared memory.
3. **Densification**: each pixel's flow = weighted average of the ≤ 9 patch vectors
   ...
   classic mistake that produces rubber-sheet output.
4. **Smoothing**: one 3×3 edge-aware (bilateral on luma gradient) blur of the flow field.
   Skip the paper's full variational refinement in v1 — measure first; it is the difference
   between 2 ms and 10 ms and mostly helps large untextured regions, rare in game footage.
...
uB(x) = (1−φ) · B_scaled(x)     // sample B at x + uB
...
1. Analytic: translating/rotating checkerboard and Perlin textures with known flow —
   ...
   the analytic occlusion of a sliding square to ≥ 90% IoU.
