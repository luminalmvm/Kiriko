...
| 4.7 | `[`/`]`, Alt+[/], Ctrl+D | Not implemented | Unbound; duplicate via menu only | ◑ Partial · ✅ CI-green · bound **Cmd/Ctrl+D** → duplicate the selected layer (in `global_shortcuts`; only consumed when a layer is selected). `[`/`]` (trim in/out to playhead) and Alt+`[`/`]` still want the underlying trim actions |
