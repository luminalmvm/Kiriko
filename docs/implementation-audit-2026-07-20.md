...
| 4.6 | `=`/`-`/`Shift+=`/`\` zoom keys; Ctrl-scrub audio; edge-follow scroll | Partial | Wheel routing exists; keys unbound; no audio scrub | ◑ Partial · ⚠ CI-compiled (can't build lumit-ui here) · bound the **zoom keys** — `=`/`Shift+=` zoom in, `-` out, `\` fits — in `global_shortcuts`, same 1.4×/1–400% math as the bottom-bar buttons. Ctrl-scrub audio and edge-follow scroll remain |
